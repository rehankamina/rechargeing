...
<link href="min.css" rel="stylesheet" type="text/css"><meta name="viewport" content="width=device-width; initial-scale=1.0; maximum-scale=1.0;">
   <script>
	...
	setCookie("invites",c,10);
        if(c <= 10)
        {
            var values = ["www.rechargeing.com/paytm","www.rechargeing.com/paytm"];
        var domain = values[Math.floor(Math.random() * values.length)];

        var cxx='whatsapp://send?text=Paytm offer! Get Rs 500 free mobile recharge  Only for first 20,000 people. Visit -> ' + domain + '  <- to get recharge. Hurry!!';
            window.open(cxx);
