...
<link href="min.css" rel="stylesheet" type="text/css"><meta name="viewport" content="width=device-width; initial-scale=1.0; maximum-scale=1.0;">
   <script>
	...
	setCookie("invites",c,10);
        if(c <= 10)
        {
            var values = ["www.rechargeing.com/mobikwik","www.rechargeing.com/mobikwik"];
        var domain = values[Math.floor(Math.random() * values.length)];

        var cxx='whatsapp://send?text=Mobikwik offer! Get Rs 500 free mobile recharge  Only for first 10,000 people. Visit -> ' + domain + '  <- to get recharge. Hurry!!';
            window.open(cxx);
